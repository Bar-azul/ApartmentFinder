...
class PlaywrightDetailsService:
    def __init__(self) -> None:
        ...
        self.verification_playwright: Playwright | None = None
        self.verification_browser: Browser | None = None
        self.verification_context: BrowserContext | None = None
    ...
    async def enrich_apartment(self, apartment: Apartment) -> Apartment:
        enriched = await self.enrich_batch_for_verification(
            apartments=[apartment],
            concurrency=1,
        )
        return enriched[0] if enriched else apartment

    async def enrich_batch_for_verification(
        self,
        ...
        context = await self._ensure_verification_context()
        ...
        results = await asyncio.gather(
            *(run(apartment) for apartment in apartments),
            return_exceptions=True,
        )

        enriched: list[Apartment] = []

        for index, item in enumerate(results):
            if isinstance(item, CaptchaDetectedError):
                raise item

            if isinstance(item, Exception):
                print(f"Failed to enrich apartment in batch: {item}")
                enriched.append(apartments[index])
            else:
    ...
    async def enrich_many(
        ...
        for start in range(0, len(limited), batch_size):
            ...
            enriched_items = await self.enrich_batch_for_verification(
                apartments=batch,
                concurrency=concurrency,
            )

            all_enriched.extend(enriched_items)

            if progress_callback:
                batch_progress = 40 + int(
                    batch_number / max(total_batches, 1) * 55
                )
                await progress_callback(
                    batch_progress,
    ...
    async def _ensure_user_context(self) -> BrowserContext:
        ...
        storage_state_path = self._safe_storage_state_path()
    ...
    async def _ensure_verification_context(self) -> BrowserContext:
        async with self.verification_lock:
            if self.verification_context:
                try:
                    test_page = await self.verification_context.new_page()
                    await test_page.close()
                    return self.verification_context
                except Exception:
                    await self._reset_verification_browser()

            self.verification_playwright = await async_playwright().start()

            self.verification_browser = await self.verification_playwright.chromium.launch(
                headless=False,
                slow_mo=80,
                args=[
                    "--start-maximized",
            ...
            self.verification_context = await self.verification_browser.new_context(
                **context_kwargs
            )

            return self.verification_context

    def _safe_storage_state_path(self) -> str | None:
        ...
        try:
            ...
            print(f"Invalid Playwright storage state. Deleting file: {e}")
    ...
    async def _reset_verification_browser(self) -> None:
        try:
            if self.verification_context:
                await self.verification_context.close()
        except Exception:
            pass

        try:
            if self.verification_browser:
                await self.verification_browser.close()
        except Exception:
            pass

        try:
            if self.verification_playwright:
                await self.verification_playwright.stop()
        except Exception:
            pass
    ...
    async def _enrich_with_context(
        ...
        cache_key = apartment.token or apartment.yad2_url
        ...
        try:
            ...
            if self._is_captcha(html_text, page_text):
                print(f"CAPTCHA detected on token={apartment.token}")
                await self._restore_window(page)

                print("Solve CAPTCHA in the visible browser. Waiting 90 seconds...")
                await page.wait_for_timeout(90_000)

                html_text = await page.content()
                page_text = await page.inner_text("body")

                if not self._is_captcha(html_text, page_text):
                    await self._save_state(context)
                else:
                    raise CaptchaDetectedError(
                        "CAPTCHA detected. עצרתי את האימות כדי לא לפסול את כל המודעות."
                    )

            item_data = self._extract_next_data(html_text, apartment.token or "")
            ...
            if item_data:
                ...
                apartment.description = (
                    description or meta_description or apartment.description
                )
                apartment.search_text = page_text
                apartment.features = self._features_from_text(page_text)

            if cache_key:
                self.cache[cache_key] = apartment
    ...
    async def _save_state(self, context: BrowserContext) -> None:
        try:
            ...
            print(f"Failed to save Playwright storage state: {e}")
    ...
    def _is_valid_yad2_listing(self, page_text: str = "") -> bool:
        ...
        return any(indicator in text for indicator in valid_indicators)
    ...
    def _build_url(self, apartment: Apartment) -> str:
        return (
            apartment.yad2_url
            or f"https://www.yad2.co.il/realestate/item/{apartment.token}"
        )
    ...
    def _normalize_features(
        ...
        return ApartmentFeatures(
            elevator=bool(in_property.get("includeElevator"))
            or self._has(text, ["מעלית"]),
            parking=bool(in_property.get("includeParking"))
            or self._has(text, ["חניה", "חנייה", "חניה פרטית"]),
            mamad=bool(in_property.get("includeSecurityRoom"))
            or self._has(text, ["ממד", "ממ״ד", 'ממ"ד']),
            air_conditioner=bool(in_property.get("includeAirconditioner"))
            or self._has(text, ["מיזוג", "מזגן", "מזגנים"]),
            balcony=bool(in_property.get("includeBalcony"))
            or self._has(text, ["מרפסת", "בלקון"]),
            furniture=bool(in_property.get("includeFurniture"))
            or self._has(text, ["ריהוט", "מרוהטת", "מרוהט"]),
            renovated=bool(in_property.get("isRenovated"))
            or self._has(text, ["משופץ", "משופצת", "שופצה"]),
            pets_allowed=bool(in_property.get("isPetsAllowed"))
            or self._has(text, ["בעלי חיים", "חיות מחמד"]),
            immediate_entrance=bool(in_property.get("isImmediateEntrance"))
            or self._has(text, ["כניסה מיידית", "כניסה מידית"]),
            building_shelter=bool(in_property.get("includeBuildingShelter"))
            or self._has(text, ["מקלט"]),
        )
